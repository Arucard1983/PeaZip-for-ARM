...
uses
  ...
  pea_utils,ansiutf8_utils, Buttons, ButtonPanel;
...
var
  FormKF: TFormKF;
  persistent_source,txt_keyfile_notcreated,txt_error_openfile,st:utf8string;
...
procedure gen_pw(keyarr:TKey2048; pwsize:integer; var pw:utf8string);
var
   i,j:integer;
   spchar:boolean;
begin
...
if SaveDialog1.execute then
   ...
   uassignfile(keyf,keyf_name);
   rewrite(keyf);
   end
else
   begin
   MessageDlg(txt_keyfile_notcreated, mtWarning, [mbOK], 0);
   exit;
   end;
read_rand(keyarr);
gen_rand(keyarr);
...
get_fingerprint (fingerprint,false);
   //MessageDlg(txt_keyfile_created+' '+keyf_name, mtInformation, [mbOK], 0);//***
...
if OpenDialog2.Execute then
   if OpenDialog2.Filename<>'' then
      ...
      uassignfile(f,OpenDialog2.Filename);
      filemode:=0;
      reset(f);
...
MessageDlg(txt_error_openfile, mtError, [mbOK], 0);
end;
end;
...
begin
MessageDlg(Imageinfoarchive1.Hint, mtInformation, [mbOK], 0);
end;
